...
def save_url(img_url,lon,lat,angle,file_name):
    handle = open(file_name,'a+')
    handle.write(img_url+' '+str(lon)+' '+str(lat)+' '+str(angle))
    handle.write('\n')
    handle.close()
        ...
        for i in coor_list:
            ...
            if len(dic['features']) > 0:
                for info in dic['features']:
                    ...
                    angle = info['properties']['ca']
                    lon = coor[0]
                    lat = coor[1]
                    image_URL = image_URL_L + key + image_URL_R
                    save_url(image_URL,lon,lat,angle,filename)
                    # print image_URL+' '+str(lon)+','+str(lat)
                    img = requests.get(image_URL)
                    f = open('.\\data\\'+key+'.jpg','ab')
                    f.write(img.content)
                    f.close()
            else:
